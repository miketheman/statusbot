...
def test_handler_status_check(mocker, apiai_event):
    ...
    assert json.loads(response["body"])["fulfillment_text"] == "The status of github site is good"
...
def test_handler_status_unknown_site(mocker, apiai_event_unimplemented):
    ...
    assert response["statusCode"] == 200
    assert response["headers"]["Content-type"] == "application/json"
    assert json.loads(response["body"])["fulfillment_text"] == "The 'example.com' site has not yet been implemented."
